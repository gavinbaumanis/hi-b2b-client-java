...
**`hi-wsdl-java`** uses the **same branch names** and the **same Maven version** on each pair (`hi-wsdl` **8.0.0.1** on `java-8` with **`hi-b2b-client`** **8.0.0.1**, and so on). Artifact ids stay **`hi-wsdl`** and **`hi-b2b-client`**; the version distinguishes the Java SE line.
...
**This checkout (`8.0.0.1-SNAPSHOT`):** Java **8**, **14** facades, types from **`au.gov.nehta:hi-wsdl`** at **`${project.version}`** (no in-repo **`wsimport`**). Stack, Surefire includes, and **`.github/workflows/ci.yml`** (branch **`java-8`**, JDK **8**) below apply to **this line only**. Other branches keep their own **`pom.xml`**, CI branch filter, and JDK.
...
Git/SCM settings for **`maven-release-plugin`** live in **`pom.xml`** properties (**`scm.repo.url`**, **`release.*`**). Tags default to **`{artifactId}-{version}`** (e.g. **`hi-b2b-client-8.0.0.1`**).
...
```text
mvn -B "-Prelease" release:prepare release:perform -DreleaseVersion=8.0.0.1 -DdevelopmentVersion=8.0.0.2-SNAPSHOT -Dtag=hi-b2b-client-8.0.0.1
```
...
- **`nehta.lib.version`** - **`au.gov.nehta:common-library`** (**8.0.0.1**; SNAPSHOT while unpublished). **`nehta.xsp.lib.version`** - **`au.gov.nehta:smi-xsp`**. **`smi-common-utils`** is not a direct dependency; **`ArgumentUtils`** and **`CertificateValidator`** come from **`smi-xsp`** / **`common-library`** transitives.
