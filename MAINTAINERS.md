...
**This 1.7.1.1 line (`1.7.1.1-SNAPSHOT`):** Java 11, Jakarta, **26** facades, in-repo **`wsimport`**. **`hi-b2b-client`** **1.7.1.1** uses **`hi-wsdl`** at the **same version**.
...
Branch model: **`1.7.1.1`**, Git **`java-11`**, full MCA, in-repo **`wsimport`**. JDK 11, Jakarta XML Web Services (**`jaxws-rt` 4.x**).
...
Keep **`hi.wsdl.version`** at **`${project.version}`** so **`hi-b2b-client`** **1.7.1.1** consumes **`hi-wsdl`** **1.7.1.1** (and **1.6.4.1** with **1.6.4.1** on the Java 8 line). Align **`jaxws-rt`**, **`jaxws-maven-plugin`**, and **`jaxb-xjc`** with **hi-wsdl-java** on the same line.
...
Git/SCM settings for **`maven-release-plugin`** live in **`pom.xml`** properties (**`scm.repo.url`**, **`release.*`**). Tags default to **`{artifactId}-{version}`** (e.g. **`hi-b2b-client-1.7.1.1`**).
...
```text
mvn -B "-Prelease" release:prepare release:perform -DreleaseVersion=1.7.1.1 -DdevelopmentVersion=1.7.1.2-SNAPSHOT -Dtag=hi-b2b-client-1.7.1.1
```
